...
const IntroScene = ({ data }: { data: WrappedData }) => (
  <div className="flex flex-col items-center justify-center h-full text-center space-y-6 p-8 bg-github-canvas-default text-github-fg-default">
    ...
    >
      <h2 className="text-3xl font-semibold text-github-fg-default mb-1">
        @{data.profile.login}
      </h2>
...
const PersonalityScene = ({ data }: { data: WrappedData }) => {
  const getIcon = () => {
    switch (data.busiestTime) {
      case "Night Owl":
        return <Moon className="w-24 h-24 text-github-accent-fg" />;
      case "Early Bird":
        return <Sun className="w-24 h-24 text-github-attention-fg" />;
      case "Lunchtime Coder":
        return <Coffee className="w-24 h-24 text-github-danger-fg" />;
      default:
        return <Laptop className="w-24 h-24 text-github-fg-default" />;
    }
  };
  ...
  <div className="flex flex-col items-center justify-center h-full text-center space-y-6 p-8 bg-github-canvas-default text-github-fg-default">
    <div className="w-full max-w-sm bg-github-canvas-subtle border border-github-border-default rounded-lg overflow-hidden shadow-lg">
      <div className="p-4 border-b border-github-border-default flex items-center gap-3">
        <img
          src={data.profile.avatar_url}
          className="w-10 h-10 rounded-full border border-github-border-default"
          alt=""
        />
        <div className="text-left">
          <div className="font-bold text-github-fg-default text-sm">
            @{data.profile.login}
          </div>
      ...
      <div className="p-4 grid grid-cols-2 gap-y-4 gap-x-2 text-left">
        <div>
          <div className="text-xs text-github-fg-muted mb-0.5">
            Top Language
          </div>
          <div className="text-sm font-semibold text-github-fg-default truncate">
        ...
        <div>
          ...
          <div className="text-sm font-semibold text-github-fg-default">
            {data.totalCommits}
          </div>
        </div>
        ...
        <div>
          ...
          <div className="text-[10px] text-github-fg-muted">
            {data.mostProductiveMonth.count} commits
          </div>
        </div>
        <div>
          ...
          {scenes.map((_, i) => (
            ...
            >
              <motion.div
                ...
                transition={{
                  duration: i === currentScene ? scenes[i].duration / 1000 : 0,
                  ease: "linear",
                }}
